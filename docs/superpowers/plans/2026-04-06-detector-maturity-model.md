...
| yaml | Built-in `python-conda-environment` rule for `environment.yml` and `environment.yaml`; checks for a top-level `dependencies` key without extracting individual entries | No | 2 |
...
After the table, add these short notes:

```md
Built-in `yaml` support is currently level 2 via the presence-check `python-conda-environment` rule. Custom `yaml` rules can remain level 2 for presence checks or reach level 3 when they extract dependency data.
