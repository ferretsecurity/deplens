...
JSON output contains a top-level `root` plus `manifests`. Each manifest entry includes `type`, `path`, optional `dependencies`, `has_dependencies`, and optional `warnings`. Each dependency is emitted as an object with a required `raw` string (the free-form value from the underlying file) plus optional structured fields: `name` (bare package identifier), `version` (resolved, for lockfiles), `constraint` (source manifest range), `section`, `source` (`registry`, `git`, `path`, `url`), and `extras` (string metadata). See [docs/superpowers/specs/2026-04-27-dependency-struct.md](docs/superpowers/specs/2026-04-27-dependency-struct.md) for field contracts. `has_dependencies` is `null` when the detector cannot determine dependency presence, `true` when extraction confirmed at least one dependency, and `false` when a detector or extractor conclusively matched but found none.
...
| package lock | `package-lock.json`; extracts versioned root project dependencies from lockfile version 1 `dependencies`, and from lockfile version 2 or 3 root-package `packages[""]` `dependencies`, `devDependencies`, and `optionalDependencies` (deduped by package name, first section wins). Also used for `npm-shrinkwrap.json` with the same root dependency extraction behavior | Yes | 3 |
